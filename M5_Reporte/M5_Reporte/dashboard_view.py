...
class DashboardView(tk.Toplevel):
    def __init__(self, parent, user_doc):
        ...
        self.parent = parent
        self.user = user_doc
        self.title(f"Dashboard - {self.user.get('usuario')} ({self.user.get('rol')})")
        ...
        lbl_title = tk.Label(header, text=f"Bienvenido, {self.user.get('usuario')}", bg=C_HEADER, fg='white',
                             font=('Segoe UI', 14, 'bold'))
        lbl_title.pack(side='left', padx=18)
        lbl_info = tk.Label(header, text=f"Rol: {self.user.get('rol')}   •   {datetime.now().strftime('%Y-%m-%d')}",
        ...
        allowed = ROLE_PERMISSIONS.get(self.user.get('rol'), [])
        if not allowed:
    ...
    def open_module(self, module_id: int):
        ...
        if module_id == 5:
            try:
                base_dir = os.path.abspath(os.path.dirname(__file__))
                script_path = os.path.join(base_dir, 'main_m5.py')
                if not os.path.exists(script_path): script_path = os.path.abspath('main_m5.py')
                if not os.path.exists(script_path):
                    messagebox.showerror('Error', f'No se encontró main_m5.py')
                    return
                python_exec = sys.executable or 'python'
                subprocess.Popen([python_exec, script_path], cwd=os.path.dirname(script_path))
                messagebox.showinfo('Módulo abierto', f'{mod_name} se abrió en una nueva ventana.')
            except Exception as e:
                messagebox.showerror('Error al abrir módulo', str(e))
        else:
            messagebox.showinfo('Abrir módulo', f'Abriendo: {mod_name}\n(En integración, aquí se llamaría la GUI del módulo.)')
